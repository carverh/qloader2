...
int validation_enabled = 1;
int invalid_syntax = 0;

int validate_line(const char *buffer) {
...
static char *config_entry_editor(const char *title, const char *orig_entry) {
	...
	int syntax_highlighting_enabled = 1;
	char *syntax_highlighting_enabled_config = config_get_value(NULL, 0, "EDITOR_HIGHLIGHTING");
    if (!strcmp(syntax_highlighting_enabled_config, "no")) syntax_highlighting_enabled = 0;
    
    validation_enabled = 1;
	char *validation_enabled_config = config_get_value(NULL, 0, "EDITOR_VALIDATION");
    if (!strcmp(validation_enabled_config, "no")) validation_enabled = 0;
...
char *menu(char **cmdline) {
    ...
    int editor_enabled = 1;
	char *editor_enabled_config = config_get_value(NULL, 0, "EDITOR_ENABLED");
    if (!strcmp(editor_enabled_config, "no")) editor_enabled = 0;
